...
@SpringUI
@Theme("mytheme")
public class LogowanieUI extends UI {
    ...
    private void rejestracja() {
        rejestracja.addClickListener(clickEvent -> {
            ...
            verticalLayout.addComponents(imieTextField, nazwiskoTextField, loginTextField, hasloField, login);
            ...
            login.addClickListener(event -> {
                ...
                if (imieText.length() > 4 && nazwiskoText.length() > 4 && loginText.length() > 4) {
                    if (hasloText.length() > 6 && hasloText.length() < 20) {
